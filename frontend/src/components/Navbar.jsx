...
/* ═══════════════════════════════════════════════════════════════
   Sidebar (barra lateral izquierda). Reemplaza la antigua barra
   horizontal. Contiene: logo, módulos, QR de reservas y perfil.
   Se mantiene el nombre "Navbar" para no tocar los imports de las
   páginas. El ancho lo compensa un margin-left global (index.css).
═══════════════════════════════════════════════════════════════ */
...
export default function Navbar() {
  ...
  // Empuja el contenido a la derecha del sidebar mientras está montado.
  // Al desmontarse (logout / páginas sin sidebar), se limpia.
  useEffect(() => {
    const root = document.getElementById('root')
    if (root) {
      root.style.paddingLeft = SIDEBAR_W + 'px'
      root.style.transition = 'padding-left 0.2s'
    }
    return () => { if (root) root.style.paddingLeft = '' }
  }, [])
  ...
  return (
    <aside style={{ position:'fixed', top:0, left:0, bottom:0, width:SIDEBAR_W, background:'var(--dark-2)', borderRight:'1px solid var(--dark-4)', display:'flex', flexDirection:'column', zIndex:100 }}>
      ...
      {/* Módulos */}
      <nav style={{ display:'flex', flexDirection:'column', gap:4, padding:'6px 14px' }}>
        {links.map(link => {
      ...
      <div ref={ref} style={{ position:'relative', borderTop:'1px solid var(--dark-4)', padding:12 }}>
        ...
        {profileOpen && (
          <div className="animate-fade-up" style={{ position:'absolute', bottom:'calc(100% + 6px)', left:12, right:12, background:'var(--dark-2)', border:'1px solid var(--dark-4)', borderRadius:12, padding:6, boxShadow:'0 8px 32px rgba(0,0,0,0.5)', zIndex:200 }}>
            {[
              { icon: Icons.settings, label:'Configuración', action:() => { navigate('/settings'); setProfileOpen(false) } },
              { icon: Icons.crown,    label:'Suscripción',   action:() => { navigate('/subscription'); setProfileOpen(false) } },
            ].map(item => (
              <button
                key={item.label}
                onClick={item.action}
