...
// Genera el mensaje pre-escrito para avisarle al barbero por WhatsApp
function buildWhatsappMessage(a) {
  const fecha = new Date(a.scheduled_at).toLocaleDateString('es-CO', {
    weekday: 'long', day: 'numeric', month: 'long', timeZone: 'America/Bogota',
  })
  const hora = new Date(a.scheduled_at).toLocaleTimeString('es-CO', {
    hour: '2-digit', minute: '2-digit', timeZone: 'America/Bogota',
  })
  const lines = [
    '¡Hola ' + (a.barber_name || '') + '! 👋',
    '',
    'Te agendaron una cita en la barbería:',
    '',
    '📅 ' + fecha[0].toUpperCase() + fecha.slice(1) + ' · ' + hora,
    '✂️ ' + a.service_name,
    '👤 Cliente: ' + a.client_name,
  ]
  if (a.client_phone) lines.push('📱 Tel: ' + a.client_phone)
  lines.push('', 'Confírmame si puedes atenderla.')
  return lines.join('\n')
}

// Normaliza el teléfono del barbero al formato que WhatsApp entiende
// (solo dígitos, con código de país). Si el número tiene 10 dígitos y
// empieza por 3, asumimos Colombia (57).
...
// Abre WhatsApp con el mensaje pre-escrito. Si el barbero no tiene
// teléfono, muestra un toast y no hace nada más.
function openWhatsappForAppointment(a, toast) {
  if (!a.barber_phone) {
    toast.error('Este barbero no tiene teléfono guardado. Agrégalo desde Barberos → Editar.')
    return
  }
  const phone = normalizeWhatsAppPhone(a.barber_phone)
          ...
          {loading ? (
            ...
            >
              ...
              <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0, marginLeft:'auto' }}>
                ...
                />
                <button
              onClick={() => openWhatsappForAppointment(a, toast)}
              title={a.barber_phone ? 'Avisar al barbero por WhatsApp' : 'El barbero no tiene teléfono — agrégalo primero'}
              style={{
                display:'inline-flex', alignItems:'center', gap:6,
                padding:'7px 12px', borderRadius:9,
                background: a.barber_phone ? '#25D366' : 'transparent',
                color: a.barber_phone ? '#0A0A0A' : 'var(--cream-dim)',
                border: a.barber_phone ? 'none' : '1px dashed var(--dark-4)',
                fontWeight:800, fontSize:11.5, letterSpacing:'0.03em',
                cursor:'pointer',
                opacity: a.barber_phone ? 1 : 0.6,
                transition:'all 0.15s',
              }}
            >
              {IcAppt.whatsapp()} Avisar
            </button>
