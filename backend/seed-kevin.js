...
async function main() {
  ...
  const shopRes = await pool.query('SELECT id, name FROM barbershops WHERE email = $1', [EMAIL])
  if (!shopRes.rows.length) {
    console.error(`✗ No existe una barbería con el correo ${EMAIL}. Aborta sin cambios.`)
    return pool.end()
  }
